...
impl<T: Animatable> AnimationSequence<T> {
    ...
    pub fn then(self, target: T, config: AnimationConfig) -> Self {
        let predicted_next = if self.steps.is_empty() {
            ...
            self.steps.last().map(|last_step| last_step.target.interpolate(&target, 0.5))
        };
...
impl<T: Animatable> Clone for AnimationSequence<T> {
    fn clone(&self) -> Self {
        Self {
            ...
            current_step: AtomicU8::new(self.current_step.load(Ordering::Relaxed)),
            on_complete: None, // Callbacks can't be cloned
        }
    }
}
...
mod tests {
    ...
    fn test_animation_sequence_builder_pattern() {
        let sequence = AnimationSequence::new()
            .then(10.0f32, AnimationConfig::new(AnimationMode::Spring(Spring::default())))
            .then(20.0f32, AnimationConfig::new(AnimationMode::Spring(Spring::default())))
            .then(30.0f32, AnimationConfig::new(AnimationMode::Spring(Spring::default())));
    ...
    fn test_animation_sequence_with_callback() {
        ...
        let mut sequence = AnimationSequence::with_on_complete(
            steps,
            move || {
    ...
    fn test_animation_sequence_clone() {
        ...
        assert_eq!(sequence1.current_step_index(), sequence2.current_step_index());
    ...
    fn test_animation_sequence_backward_compatibility() {
        ...
        let sequence = AnimationSequence::new();
        let sequence = sequence.then(10.0f32, AnimationConfig::new(AnimationMode::Spring(Spring::default())));
        let sequence = sequence.then(20.0f32, AnimationConfig::new(AnimationMode::Spring(Spring::default())));
